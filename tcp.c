...
static int
mk_hash_index(struct tuple4 addr)//tuple4 define in nids.h
{
...
add2buf(struct half_stream * rcv, char *data, int datalen)
{
  ...
  if (datalen + rcv->count - rcv->offset > rcv->bufsize) {
	...
	toalloc = datalen * 2;
      ...
      rcv->data = realloc(rcv->data, toalloc);//rte_realloc
      rcv->bufsize = toalloc;
    }
    if (!rcv->data)
...
tcp_init(void * handle, int size)
{
  ...
  impl -> streams_pool = (struct tcp_stream *) malloc((impl -> max_stream + 1) * sizeof(struct tcp_stream));
...
process_icmp(u_char * data)
{
  ...
  if (ip_compute_csum((char *) pkt, len))//checksum.c
    return;
